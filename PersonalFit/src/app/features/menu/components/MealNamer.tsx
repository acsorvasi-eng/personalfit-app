...
export function MealNamer({
  ...
  const handleGenerate = async () => {
    ...
    setLoading(true);
    try {
      const resp = await authFetch(`${apiBase}/api/meal-name`, {
        method: "POST",
        headers: {
        ...
        >
          ...
          <div className="px-4 pb-4 space-y-4">
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2 text-xs font-medium text-amber-200/80 uppercase tracking-wide">
                ...
                {GEO_OPTIONS.map((opt) => {
                  ...
                  return (
                    <button
                      ...
                      className={[
                        "px-2.5 py-2 rounded-2xl text-left border text-xs transition-all",
                        active
                ...
                {(["hu", "ro", "en"] as LanguageCode[]).map((code) => {
                  ...
                  return (
                    <button
                      ...
                      className={[
                        "px-2.5 py-1.5 rounded-2xl border text-xs flex items-center justify-center gap-1 transition-all",
                        active
            ...
            <div className="space-y-1">
              <p className="text-xs text-amber-100/80 uppercase tracking-wide">
              ...
              <div className="flex flex-wrap gap-1.5 max-h-20 overflow-y-auto pr-1">
                {ingredients.map((ing) => (
                  <span
                    key={ing}
                    className="px-2 py-0.5 rounded-full bg-black/40 border border-amber-900/60 text-xs text-amber-50"
                  >
                    {ing}
                  </span>
                ))}
              ...
              <div className="flex items-center justify-between">
                ...
                {result && (
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-2xl bg-black/40 border border-amber-500/50 text-amber-100 hover:bg-black/60 transition-colors"
                  >
