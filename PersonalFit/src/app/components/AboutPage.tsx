      ...
      <div className="flex-1 overflow-y-auto px-3 sm:px-4 lg:px-6 py-4 space-y-4">
        ...
        <DSMCard>
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="text-gray-900" style={{ fontWeight: 700 }}>{t('about.missionTitle')}</h2>
          </div>
        ...
        <DSMCard>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {TEAM_VALUES.map((val) => (
              <div key={val.title} className="flex items-start gap-3 p-3 bg-gray-50 rounded-xl">
                ...
                <div>
                  <div className="text-sm text-gray-900" style={{ fontWeight: 700 }}>{val.title}</div>
        ...
        <DSMCard>
          ...
          <div className="flex flex-wrap gap-2">
            {['React 18', 'TypeScript', 'Tailwind CSS v4', 'Motion', 'Lucide Icons', 'Recharts', 'LocalStorage', 'PWA Ready'].map(tech => (
              <span key={tech} className="px-2.5 py-1 bg-emerald-50 text-emerald-700 rounded-lg text-[11px]" style={{ fontWeight: 600 }}>
                {tech}
              </span>
            ))}
          </div>
...
function InfoRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between py-2 border-b border-gray-50 last:border-b-0">
      ...
      <span className="text-sm text-gray-900" style={{ fontWeight: 600 }}>{value}</span>
    </div>
  );
}
